...
bot.on('message', async (msg) => {
    ...
    // А также игнорируем сообщения, которые являются ответами на команды (например, нажатия на кнопки)
    // чтобы избежать двойной обработки или удаления системных сообщений
    if (text.startsWith('/') || String(chatId) !== String(CHAT_ID) || msg.reply_to_message) {
        return;
    }
    ...
    if (userStates[chatId] && userStates[chatId].state === 'awaiting_password') {
        // *** Добавляем удаление сообщения с паролем здесь ***
        try {
            await bot.deleteMessage(chatId, messageId);
            console.log(`Сообщение с паролем (ID: ${messageId}) удалено из чата ${chatId}.`);
        } catch (error) {
            console.error(`Не удалось удалить сообщение с паролем (ID: ${messageId}) в чате ${chatId}:`, error.message);
            // Обработайте ошибку, если бот не имеет прав на удаление сообщений
            // Или если сообщение слишком старое для удаления (Telegram API ограничение 48 часов)
        }
        // ****************************************************
        ...
        if (text === ADMIN_PASSWORD) {
            ...
            // Поскольку сообщение с паролем уже удалено, отправляем новое сообщение с ошибкой
