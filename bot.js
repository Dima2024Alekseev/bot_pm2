...
bot.onText(/📄 Последние (\d+) логов|📄 Последние 20 логов/, async (msg, match) => {
    ...
    readLastLines(LOG_FILE_OUT, linesToFetch, async (err, outLogs) => {
        ...
        // Сначала отправляем заголовок с Markdown
        await sendTelegramMessage(chatId, `[OUT - *${PM2_APP_NAME}* - ЗАПРОС ${linesToFetch}]`, { parse_mode: 'Markdown' });
        // Затем отправляем сам лог в кодовом блоке с MarkdownV2
        await sendTelegramMessage(chatId, `\`\`\`\n${outLogs || 'Нет записей в OUT логе.'}\n\`\`\``, { parse_mode: 'MarkdownV2' });
    });

    // Чтение логов из файлов ERR
    readLastLines(LOG_FILE_ERR, linesToFetch, async (err, errLogs) => {
        ...
        // Сначала отправляем заголовок с Markdown
        await sendTelegramMessage(chatId, `[ERR - *${PM2_APP_NAME}* - ЗАПРОС ${linesToFetch}]`, { parse_mode: 'Markdown' });
        // Затем отправляем сам лог в кодовом блоке с MarkdownV2
...
bot.on('message', async (msg) => {
    ...
    // А также пустые сообщения
    if (msg.text.startsWith('/') || String(chatId) !== String(CHAT_ID) || !text || text === '') {
        return;
    }
    ...
    // Если вы хотите отвечать на произвольный текст, добавьте здесь логику:
    // await sendTelegramMessage(chatId, 'Неизвестная команда или действие. Пожалуйста, используйте кнопки или доступные команды.', { parse_mode: 'Markdown' });
});
